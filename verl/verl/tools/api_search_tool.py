...
from verl.tools.utils.search_api_docs_utils import perform_single_search_batch
...
class APISearchTool(BaseTool):
    """
    API Search tool for retrieving API docs using external retrieval services.
    ...
    def __init__(self, config: dict, tool_schema: OpenAIFunctionToolSchema):
        ...
        Example tool_schema:
            {
                ...
                "function": {
                    ...
                    "description": " Searches relevant tools' API documents based on the given query and optional category filter.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "category": {
                                "type": "string",
                                "description": "Optional category to filter the tool's API documents. If set to None or "all", searches across all categories."
                            },
                            "query": {
                                "type": "string",
                                "description": "A fully-formed semantic query. The tool will return search results for the query."
                            }
                        },
                        "required": ["query_list"]
                    }
        ...
        logger.info(f"Initialized APISearchTool with config: {config}")
    ...
    def execute_search(self, instance_id: str, category:str, query: str, retrieval_service_url: str, topk: int, timeout: int):
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs) -> tuple[str, float, dict]:
        ...
        category_from_params = parameters.get("category")
        ...
        if not query_from_params or not isinstance(query_from_params, str):
            ...
            logger.error(f"[APISearchTool] {error_msg} Received parameters: {parameters}")
        ...
        try:
            result_text, metadata = await self.execution_pool.execute.remote(
                self.execute_search, instance_id, category_from_params, query_from_params, self.retrieval_service_url, self.topk, timeout
            )
            ...
            logger.error(f"[APISearchTool] Execution failed: {e}")
