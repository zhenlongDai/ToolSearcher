...
def call_search_api(
    ...
    payload = {"category":category, "query": query, "topk": topk, "return_scores": return_scores}
...
def _passages2class_name(retrieval_result):
    ...
    #     format_reference += f"API Doc {idx + 1}:\n{content}\n"
    # return format_reference.strip()
    #获取class_name,最后放到metainfo中用于计算召回等信息
    pass
...
def perform_single_search_batch(
    ...
    """
    logger.info(f"Starting batch search for {len(query)} queries.")
