...
class SGLangRollout(BaseRollout):
    ...
    def _initialize_tools(self, config, processing_class):
        ...
        tool_list = initialize_tools_from_config(tools_config_file) #tool对象
        #(f"Initialize tools from configuration.: tool_list: {tool_list}")
        logger.info(f"Initialize tools from configuration.: tool_list: {tool_list}")
        tool_schemas = [tool.get_openai_tool_schema().model_dump() for tool in tool_list]
        tool_map = {tool.name: tool for tool in tool_list} #seachtool map获取
        ...
        return (
            tool_schemas,
            tool_map,
            tool_call_parser_type, #qwen2.5
            sgl_tools,
            function_call_parser, #Qwen25Detector
        )
        ...
        **kwargs,
        ...
        while current_turns < self.config.multi_turn.max_assistant_turns:
            if _req.state == AsyncRolloutRequestStateEnum.PENDING:
                await self._handle_pending_state(_req) # bind tool's instance based on _req.request_id
                ...
                if finish_reason_type == FinishReasonTypeEnum.LENGTH:
                    ...
                    break
                else: # Parsing parameters then add_assistant_message
                    if self._function_call_parser and self._function_call_parser.has_tool_call(content): #check是否存在<tool_call>
        ...
        all_rewards = {**tool_reward_scores, **{"user_turn_rewards": user_turn_rewards}} #reward of each tool "name":reward
