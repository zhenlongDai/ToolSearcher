    ...
    with tempfile.TemporaryDirectory() as tmp_download_dir:
        for split in ["train", "test"]:
            ...
            try:
                ...
                # )
                local_parquet_filepath = f"/ossfs/workspace/hy65/dzl/data/tool_data/nq_hotpotqa_train/{parquet_filename}"
                print(local_parquet_filepath)
                # Load and process Parquet file
                df_raw = pd.read_parquet(local_parquet_filepath)
                print(f"Loaded {len(df_raw)} rows from {parquet_filename}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--local_dir",
        default="/ossfs/workspace/hy65/dzl/data/tool_data/searchr1/data/searchR1_processed_direct",
